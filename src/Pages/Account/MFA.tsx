...
let AccountMFA = () => {
  ...
  onMount(async () => {
    let dat = await fetch('https://id.api.phaz.uk/api/v1/account/enable_mfa', { credentials: 'include' });
  ...
  let submit = async ( code: string ) => {
    ...
    let dat = await fetch('https://id.api.phaz.uk/api/v1/account/confirm_mfa', { 
      credentials: 'include', 
  ...
  let disable = async () => {
    let dat = await fetch('https://id.api.phaz.uk/api/v1/account/disable_mfa', { credentials: 'include', method: 'DELETE' });
