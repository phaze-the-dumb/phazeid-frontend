...
let Profile = () => {
  ...
  onMount(async () => {
    ...
    let dat = await fetch('http://localhost:8080/api/v1/oauth/app?client_id=' + clientId + '&redirect_uri=' + redirectUri, { credentials: 'include' });
  ...
  let allow = async ( useTurnstile: boolean = true ) => {
    let dat = await fetch('http://localhost:8080/api/v1/oauth/authorize?response_type=' + responseType + '&client_id=' + clientId + '&redirect_uri=' + redirectUri + '&scope=' + scope, {
      credentials: 'include',
      method: 'PUT',
