...
let Profile = () => {
  ...
  let refreshPatreon = async () => {
    ...
    let dat = await fetch('http://localhost:8080/api/v1/patreon/refresh', { credentials: 'include' });
    if(dat.status !== 200){
      ...
      <div class="app-container" style={{ height: '515px' }}>
        <div>
          ...
          <Show when={patreonLinked()} fallback={
            <div class="patreon-button" onClick={() => window.open('http://localhost:8080/api/v1/patreon/link')}>Link Patreon Account</div>
          }>
            <div>
